...
import { calculateSalonForSalePrice, generatePromotionalText } from "@/utils/postingPriceCalculator";
...
const SalonPost = () => {
  ...
  }, [form.watch("isNationwide"), form.watch("fastSalePackage")]);
  
  // Generate promotional text for the current pricing
  const promotionalText = generatePromotionalText('salon', userStats, pricingOptions);
  ...
  const handleFastSaleChange = (checked: boolean) => {
    form.setValue("fastSalePackage", checked);
    if (checked) {
