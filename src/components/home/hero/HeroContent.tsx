  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex h-full items-center justify-center">
      <div className="flex flex-col items-center text-center max-w-5xl mx-auto w-full my-8 md:my-0">
        ...
        <motion.h1 
          className={`${isMobile ? 'text-3xl sm:text-4xl' : 'text-4xl sm:text-5xl md:text-6xl lg:text-7xl'} font-bold font-serif leading-tight mb-6 sm:mb-8 text-white drop-shadow-lg tracking-tight px-2`}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          {isMobile ? 'Hair, Nails, Makeup, Tattoos, Brows, Barbers—All in One App.' : 'Hair, Nails, Makeup, Tattoos, Brows, Barbers, Booth Rentals—All in One Powerful App. Finally.'}
        </motion.p>
        ...
        >
          <Link to="/auth/signup" className="w-full sm:w-auto">
            <Button 
              ...
              className={`w-full sm:w-auto font-medium ${isMobile ? 'px-4 py-3 text-base' : 'px-8 sm:px-10 py-6 sm:py-7 text-lg sm:text-xl'} bg-gradient-to-r from-[#FF5A5F] to-[#FF7E3F] hover:opacity-90 shadow-xl shadow-primary/20 hover:shadow-2xl hover:shadow-primary/30 transition-all duration-300 border-0`}
            >
              Join The Movement
            </Button>
          </Link>
          ...
          {isMobile ? (
            <Link to="/jobs" className="w-full sm:w-auto">
              <Button 
                variant="outline" 
                className="w-full sm:w-auto relative font-medium px-4 py-3 text-base overflow-hidden border-gray-300 bg-white/80 backdrop-blur-sm hover:bg-white transition-all duration-300 group"
              >
