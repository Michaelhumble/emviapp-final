...
export const SalonPaymentOptions = ({ 
  form, 
  selectedOptions, 
  onOptionsChange 
}: SalonPaymentOptionsProps) => {
  ...
  const handleFeaturedToggle = () => {
    onOptionsChange({
      ...selectedOptions,
      featuredAddon: !selectedOptions.featuredAddon
    });
  };
        ...
        {SALON_PRICING_PLANS.map((plan) => (
          <Card 
            ...
            className={`cursor-pointer transition-all border-2 ${
              selectedOptions.selectedPricingTier === plan.tier
      ...
      <Card className="border-2 border-yellow-200 bg-yellow-50">
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            ...
            <Button
              type="button"
              variant={selectedOptions.featuredAddon ? "default" : "outline"}
              onClick={handleFeaturedToggle}
            >
              {selectedOptions.featuredAddon ? "Added" : "Add"}
            </Button>
          </div>
        </CardContent>
      </Card>
