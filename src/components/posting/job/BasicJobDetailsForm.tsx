...
type BasicJobFormData = z.infer<typeof basicJobSchema>;

interface BasicJobDetailsFormProps {
  onSubmit: (data: BasicJobFormData) => void;
}

const BasicJobDetailsForm: React.FC<BasicJobDetailsFormProps> = ({ onSubmit }) => {
  const form = useForm<BasicJobFormData>({
    resolver: zodResolver(basicJobSchema),
    defaultValues: {
      salonName: '',
      jobTitle: '',
      location: '',
  ...
  const handleSubmit = (data: BasicJobFormData) => {
    onSubmit(data);
  };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        <div className="border-b pb-4">
          ...
          <p className="text-sm text-muted-foreground mt-1">Enter the basic information for your job posting</p>
        </div>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  placeholder="e.g., $20-25/hour, $800/week, $3500/month"
                  className="border-gray-300 bg-white hover:border-gray-400 focus:border-gray-500 focus:ring-1 focus:ring-gray-500"
                  {...field}
                />
              </FormControl>
...
export default BasicJobDetailsForm;
