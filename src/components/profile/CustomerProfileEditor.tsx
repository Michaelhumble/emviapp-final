...
const CustomerProfileEditor = () => {
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      // Convert preferences array to string if it exists
      if (userProfile.preferences && Array.isArray(userProfile.preferences)) {
        setPreferences(userProfile.preferences.join(", "));
      }
    }
  }, [userProfile]);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Prepare the preferences array from comma-separated string
      const preferencesArray = preferences
        ? preferences.split(',').map(pref => pref.trim()).filter(pref => pref)
        : [];
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="preferences">Beauty Preferences (optional)</Label>
                  <Input
                    id="preferences"
                    placeholder="e.g. Natural makeup, Hair coloring, Manicures"
                    value={preferences}
                    onChange={(e) => setPreferences(e.target.value)}
                  />
                  <p className="text-xs text-muted-foreground">
                    Enter your beauty preferences separated by commas
                  </p>
                </div>
              </div>
            </div>
