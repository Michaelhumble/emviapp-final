...
/**
 * Rebuilt Artist Dashboard with clean architecture
 * Completely independent from Customer Dashboard
 */
const NewArtistDashboard = () => {
  ...
  const { data: profileData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      console.log('[NewArtistDashboard] Fetching profile data for user:', user?.id);
      ...
      if (error) {
        console.error('[NewArtistDashboard] Error fetching profile:', error);
        throw error;
      }
