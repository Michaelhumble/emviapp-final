-- Fix remaining RLS policies for tables that have RLS enabled but no policies
...
-- Motivational messages - public read, authenticated users can create
CREATE POLICY "Anyone can view motivational messages" ON public.motivational_messages
FOR SELECT USING (true);

CREATE POLICY "Authenticated users can create messages" ON public.motivational_messages
...
-- Salon views - public can view, authenticated users can track views
...
-- Staff service assignments - salon owners and staff can manage
CREATE POLICY "Salon staff can view their assignments" ON public.staff_service_assignments
FOR SELECT USING (
  auth.uid() = staff_member_id OR
  auth.uid() IN (SELECT owner_id FROM salons s JOIN salon_staff ss ON s.id = ss.salon_id WHERE ss.id = staff_member_id)
);

CREATE POLICY "Salon owners can manage staff assignments" ON public.staff_service_assignments
FOR ALL USING (auth.uid() IN (
  SELECT s.owner_id FROM salons s 
  JOIN salon_staff ss ON s.id = ss.salon_id 
  WHERE ss.id = staff_member_id
));