...
-- STEP 2: Update the existing profiles table to add missing columns
...
-- Update the role constraint to include all allowed roles
...
-- STEP 3: Temporarily remove foreign key constraints that might have orphaned data
...
-- STEP 4: Clean up orphaned data from activity_log
...
-- STEP 6: Create new trigger function for profiles table
...
-- STEP 8: Update existing profiles with email and role from auth.users
UPDATE public.profiles 
SET 
  email = COALESCE(au.email, profiles.email),
  role = COALESCE(au.raw_user_meta_data->>'role', 'customer'),
  full_name = COALESCE(profiles.full_name, split_part(au.email, '@', 1))
FROM auth.users au 
WHERE profiles.id = au.id;

-- STEP 9: Create profiles for any auth.users that don't have profiles yet
...
-- STEP 10: Re-add foreign key constraints now that profiles exist
...
-- STEP 11: Create/Update RLS policies for profiles table
