-- ========== WEEK-2 SECURITY HARDENING (SAFE, ADDITIVE) ==========

-- 0) Safety: run in a transaction so we can rollback if needed.
BEGIN;
...
create policy if not exists "nails_public_read"
on storage.objects
for select
using (bucket_id = 'nails');

-- Only authenticated users can INSERT into their own folder
create policy if not exists "nails_insert_own_folder"
on storage.objects
for insert
to authenticated
with check (
  bucket_id = 'nails'
  and name like auth.uid()::text || '/%'  -- name column is the object key inside bucket
);

-- Only owner can UPDATE their own files
create policy if not exists "nails_update_own"
on storage.objects
for update
to authenticated
using (
...
create policy if not exists "nails_delete_own"
on storage.objects
for delete
to authenticated
using (
...
-- 4) FINAL SWEEP: ensure our SECURITY DEFINER functions keep a safe search_path
do $$
declare r record;
begin
  for r in
