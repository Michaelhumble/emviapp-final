// Public Edge Function to serve Salons sitemap and daily shards
// Routes:
//  - /salons-sitemap.xml (index)
//  - /salons-sitemaps/sitemap-YYYY-MM-DD.xml (daily shard)
...
async function fetchActiveSalonsUpdatedOn(dateStr?: string) {
  let query = supabase
    .from('salon_sales')
    .select('id, updated_at, status, salon_name, city, state')
    .eq('status', 'active')
  ...
  // Only return salons that have required fields for valid URLs
  return (data || []).filter(salon => 
    salon.id && 
    salon.salon_name && 
    salon.salon_name.trim() !== '' &&
...
async function handleIndex(url: URL) {
  ...
  const shardPath = `${BASE_URL}/salons-sitemaps/sitemap-${y}-${m}-${d}.xml`;
...
async function handleDaily(dateStr: string) {
  const salons = await fetchActiveSalonsUpdatedOn(dateStr);

  const urls = salons.map((s: any) => {
    const loc = `${BASE_URL}/salons/${s.id}`;
    const lastmod = formatDate(s.updated_at || new Date());
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isIndex = (
      pathname === '/salons-sitemap' ||
      pathname.endsWith('/salons-sitemap') ||
      pathname.endsWith('/salons-sitemap.xml')
    );

    let dateMatch: RegExpMatchArray | null = null;
    if (pathname.includes('/salons-sitemaps/')) {
    ...
    // HEAD support: return same headers with 200
    if (req.method === 'HEAD') {
    ...
    console.error('Salons sitemap error', e);
