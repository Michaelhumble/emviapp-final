 ...
 * 
 * Tests the HubSpot integration by creating/updating test contacts and deals
 * with sample attribution data. Does not require premium scopes.
 * 
 * Usage: node scripts/hubspot-dry-run.mjs
 */
...
async function testContactUpsert() {
  ...
  const contactProperties = {
    ...
    landing_url: 'https://www.emvi.app/jobs/san-francisco/nails?utm_source=google',
    first_seen_at: Date.now().toString(),
    press_slug: 'beauty_startup_funding',
  ...
  try {
    ...
    if (searchResult.results && searchResult.results.length > 0) {
      ...
      const { data: updateResult } = await hubspotRequest(`/crm/v3/objects/contacts/${contactId}`, {
        method: 'PATCH',
 ...
 */
async function testDealCreation(contactId) {
...
/**
 * Test form submission using Forms API
 */
async function testFormSubmission() {
  console.log('📋 Testing Form Submission...');
  
  // Use a generic form ID (you may need to create this in your HubSpot portal)
  const formId = 'test-form-id'; // Replace with actual form ID
  
  const formData = {
    fields: [
      { name: 'email', value: TEST_EMAIL },
      { name: 'firstname', value: 'Emvi' },
      { name: 'lastname', value: 'Test User' },
      { name: 'message', value: 'This is a test form submission from the dry run script' },
      { name: 'utm_source', value: 'google' },
      { name: 'utm_campaign', value: 'dry_run_test' }
    ],
    context: {
      pageUri: 'https://www.emvi.app/contact',
      pageName: 'Contact Form - Dry Run Test'
    }
  };

  try {
    const { data: result } = await hubspotRequest(`/submissions/v3/integration/submit/${PORTAL_ID}/${formId}`, {
      method: 'POST',
    ...
    console.warn(`  ⚠️  Form submission failed (this is expected if form ID doesn't exist): ${error.message}`);
    return null;
  }
}
